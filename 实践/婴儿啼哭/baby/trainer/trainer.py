...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for noisy_mag, clean_mag, mask, n_frames_list in self.train_dataloader:
            self.optimizer.zero_grad()

            noisy_mag = noisy_mag.to(self.device)
            clean_mag = clean_mag.to(self.device)

            # print(f"noisy_mag devices is :{noisy_mag.device}")
            # print(f"self.model devices is :{self.model.device}")
            pred_mask = self.model(noisy_mag)
            # loss = self.loss_function(pred_mask, mask, n_frames_list)  # IRM

            loss = self.loss_function(pred_mask, clean_mag, n_frames_list)
    ...
    def _validation_epoch(self, epoch):
        ...
        for i, (noisy_y, clean_y, name) in enumerate(self.validation_dataloader):
            ...
            noisy_mag, noisy_phase = librosa.magphase(librosa.stft(noisy_y, n_fft=320, hop_length=160, win_length=320)) #修改n_fft
            ...
            # pred_clean_mag_tensor = noisy_mag_tensor * pred_mask  # IRM
            pred_clean_mag_tensor = pred_mask
            ...
            if i <= visualize_audio_limit:
                ...
                self.writer.add_audio(f"Speech/{name}_Clean", clean_y, epoch, sample_rate=16000)
                self.writer.add_audio(f"Speech/{name}_Denoisy", pred_clean_y, epoch, sample_rate=16000)
