...
class NewSublayerConnection(nn.Module):
    def __init__(self, size, dropout):
        super(NewSublayerConnection, self).__init__()
        # 去除LayerNorm
    ...
    def forward(self, x, sublayer):  # sublayer是由block实现的，属于transformer中自己实现的
        ...
        return x + self.dropout(sublayer(x)) * self.resweight
...
class LocalRNN(nn.Module):
    def __init__(self, input_dim , output_dim, rnn_type, ksize, dropout):
        ...
        print(f"=========================================")
        print(f"self.rnn={self.rnn.device}")
        # To speed up
        idx = [i for j in range(self.ksize-1,10000,1) for i in range(j-(self.ksize-1),j+1,1)]
        self.select_index = torch.LongTensor(idx).cuda()
        self.select_index = self.select_index.to(self.rnn.device)
        print(f"self.select_index.device={self.select_index.device}")

        self.zeros = torch.zeros((self.ksize-1, input_dim)).cuda()
        self.zeros = self.zeros.to(self.rnn.device)

        print(f"self.zeros.device={self.zeros.device}")
...
class LocalRNNLayer(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, rnn_type, ksize, dropout):
        ...
        self.connection = NewSublayerConnection(output_dim, dropout)
        # self.connection = SublayerConnection(output_dim, dropout)

    def forward(self, x):
        "Follow Figure 1 (left) for connections."
        x = x.to("cuda:1")
        x = self.connection(x, self.local_rnn).to("cuda:0")
        return x
...
class Block(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, rnn_type, ksize, N, h, dropout):
        ...
        self.connections = clones(NewSublayerConnection(output_dim, dropout), 2)
