...
class MetaRNNCell(MetaRNNCellBase):
    def __init__(self, input_size, hidden_size, hyper_hidden_size, hyper_embedding_size, bias=True, bias_hyper=True):
        ...
        self.weight_iH = Parameter(torch.Tensor(hidden_size, input_size)).cuda()
        self.weight_HH = Parameter(torch.Tensor(hidden_size, hidden_size)).cuda()

        self.weight_ih = Parameter(torch.Tensor(hyper_hidden_size, input_size)).cuda()
        self.weight_hh = Parameter(torch.Tensor(hyper_hidden_size, hyper_hidden_size)).cuda()
        self.weight_Hh = Parameter(torch.Tensor(hyper_hidden_size, hidden_size)).cuda()

        self.weight_hzi = Parameter(torch.Tensor(hyper_embedding_size, hyper_hidden_size)).cuda()
        self.weight_hzH = Parameter(torch.Tensor(hyper_embedding_size, hyper_hidden_size)).cuda()
        self.weight_hzb = Parameter(torch.Tensor(hyper_embedding_size, hyper_hidden_size)).cuda()
        self.weight_dziH = Parameter(torch.Tensor(hidden_size, hyper_embedding_size)).cuda()
        self.weight_dzHH = Parameter(torch.Tensor(hidden_size, hyper_embedding_size)).cuda()
        self.weight_bzH = Parameter(torch.Tensor(hidden_size, hyper_embedding_size)).cuda()
        if bias:
            self.bias_i = Parameter(torch.Tensor(hyper_embedding_size)).cuda()
            self.bias_H = Parameter(torch.Tensor(hyper_embedding_size)).cuda()
            self.bias = Parameter(torch.Tensor(hidden_size)).cuda()
        else:
            self.register_parameter('bias', None)
        if bias_hyper:
            self.bias_hyper = Parameter(torch.Tensor(hyper_hidden_size)).cuda()
        else:
...
class MetaLSTMCell(MetaRNNCellBase):
    ...
    # @torchsnooper.snoop()
    def forward(self, input, state):
        # state = { tensor<(8, 161), float32, cuda:0}
        main_state = state[0]
        meta_state = state[1]
        main_h = main_state[0]
        main_c = main_state[1]
        ...
        meta_i = torch.sigmoid(meta_pre[:, : self.hyper_hidden_size])
        meta_f = torch.sigmoid(meta_pre[:, self.hyper_hidden_size: self.hyper_hidden_size * 2])
        meta_g = torch.tanh(meta_pre[:, self.hyper_hidden_size * 2: self.hyper_hidden_size * 3])
        meta_o = torch.sigmoid(meta_pre[:, self.hyper_hidden_size * 3: ])
        meta_c = meta_f * meta_c + meta_i * meta_g
        meta_h = meta_o * torch.tanh(meta_c)
        ...
        main_i = torch.sigmoid(pre[:, : self.hidden_size])
        main_f = torch.sigmoid(pre[:, self.hidden_size: self.hidden_size * 2])
        main_g = torch.tanh(pre[:, self.hidden_size * 2: self.hidden_size * 3])
        main_o = torch.sigmoid(pre[:, self.hidden_size * 3:])
        main_c = main_f * main_c + main_i * self.dropout(main_g)
        main_h = main_o * torch.tanh(main_c)
