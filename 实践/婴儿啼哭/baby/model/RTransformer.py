...
class LayerNorm(nn.Module):   
    ...
    def __init__(self, features, eps=1e-6):
        ...
        self.b_2 = nn.Parameter(torch.zeros(features))                       #  加这个值
        self.eps = eps
...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, d_model, dropout=0.1):
        ...
        d_ff = d_model * 4
        self.w_1 = Conv(d_model, d_ff,kernel_size = 3,stride = 1, padding = 1,w_init='relu')
        self.w_2 = Conv(d_ff, d_model,kernel_size = 3,stride = 1, padding = 1,w_init='relu')
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x):
        ...
        x = x + input_
        x = self.dropout(x)
        # return self.w_2(self.dropout(x + F.relu(self.w_1(x))))
        return x
...
class MHPooling(nn.Module):
    def __init__(self, d_model, h, dropout=0.1):
        ...
        self.mask = (torch.from_numpy(subsequent_mask) == 0).unsqueeze(1).cuda() # 删除cuda
        print(f"self.mask.device={self.mask.device}")
...
class LocalRNN(nn.Module):
    def __init__(self, input_dim , output_dim, rnn_type, ksize, dropout):
        ...
        # To speed up
        idx = [i for j in range(self.ksize-1,1000,1) for i in range(j-(self.ksize-1),j+1,1)]
        self.select_index = torch.LongTensor(idx).cuda()              # 删除 cuda()
        

        self.zeros = torch.zeros((self.ksize-1, input_dim)).cuda()   # 删除 cuda()
    ...
    # @torchsnooper.snoop()
    def get_K(self, x):
    # def get_K(self, x, x_device):
        batch_size, l, d_model = x.shape
        # self.zeros = self.zeros.to(x_device)
        # self.select_index = self.select_index.to(x_device)
