...
model = MyNet()
out = model.net(input=image, class_dim=N_CLASS)                                           # 输出
...
place = fluid.CUDAPlace(0)                             # place = fluid.CPUPlace() CPU的使用
exe = fluid.Executor(place)                                                              # 
exe.run(fluid.default_startup_program())                                                 # exe.run
# fluid.io.load_params(executor=exe, dirname=model_save_dir,
#                     main_program=None)
feeder = fluid.DataFeeder(place=place, feed_list=[image, label])                         # feeder
