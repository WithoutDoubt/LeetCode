...
class Conv2D(fluid.dygraph.Layer):
    def __init__(self, name_scope, num_channels, num_filters, filter_size, stride=1,
        ...
        self._conv2d = fluid.dygraph.Conv2D(
            ...
            param_attr=param_attr, bias_attr=bias_attr, act=act, use_cudnn=use_cudnn
        )
...
class Conv2DPool(fluid.dygraph.Layer):
    def __init__(self, name_scope, num_channels, num_filters, filter_size, pool_size, pool_stride,
                 ...
                 act=None, use_cudnn=False, param_attr=None, bias_attr=None):
...
class AlexNet(fluid.dygraph.Layer):
    def __init__(self, name_scope, class_dim):
        ...
        self.conv_pool_1 = Conv2DPool(self.full_name(), 3, 64, 11, 3, 2, conv_stride=4, conv_padding=2,
                                      act='relu')
        self.conv_pool_2 = Conv2DPool(self.full_name(), 64, 192, 5, 3, 2, conv_stride=1, conv_padding=2,
                                      act='relu')
        self.conv_3 = Conv2D(self.full_name(), 192, 384, 3, 1, 1, act='relu')
        self.conv_4 = Conv2D(self.full_name(), 384, 256, 3, 1, 1, act='relu')
        self.conv_pool_5 = Conv2DPool(self.full_name(), 256, 256, 3, 3, 2, conv_stride=1, conv_padding=1,
                                      act='relu')

        self.fc6 = fluid.dygraph.Linear(9216, 4096, act='relu')
        self.fc7 = fluid.dygraph.Linear(4096, 4096, act='relu')
        self.fc8 = fluid.dygraph.Linear(4096, class_dim, act='softmax')
    ...
    def forward(self, inputs, label=None):
        ...
        out = fluid.layers.reshape(out, shape=[-1, 9216])
        out = self.fc6(out)
